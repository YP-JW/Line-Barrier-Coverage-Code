	...
	while(M!=0)
	{
		...
		else
		{
			...
			for(i=acount+1;i<=mid;i++)
			{
				...
				M=M-floor(L1[h].mij/2)-1;
				L1[h].mij=ceil(L1[h].mij/2)-1;
				fl(i);
			}
		}
...
void cal_D2()
{
	...
	for(i=0;i<S.size();i++)
	{
		...
		for(j=0;j<S.size();j++)
		{
			t2=S[j];q1=ceil((l2[t2]-g2[t1])/(2*r));q3=floor((l1[t2]-g1[t1])/(2*r));
			//cout<<l2[t2]<<" "<<g2[t1]<<" "<<q1<<" "<<q2<<" "<<q3<<endl;
			if(l1[t2]>=g1[t1]&&q1==q3)
			{
...
int main()
{
	...
	int temp,i;
	char tmp[100];
	freopen("2.BCT_result.txt","w",stdout);
	...
	mx.clear();S1.clear();S2.clear();mi.clear();v.clear();Dd.clear();acount=0,acount1=0;
